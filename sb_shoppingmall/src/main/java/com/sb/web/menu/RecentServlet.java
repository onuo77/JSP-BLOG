...
public class RecentServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {		
		...
		List<Product> products = listDao.getProductsThisMonthUpToDate(param);

		System.out.println(products);
		
		///
		int totalRows = listDao.getProductsThisMonthTotalRows(param);
		// 전체페이지
		...
		//event.jsp로 내부이동
		request.getRequestDispatcher("/WEB-INF/views/menus/recent.jsp").forward(request, response);
	}
}
