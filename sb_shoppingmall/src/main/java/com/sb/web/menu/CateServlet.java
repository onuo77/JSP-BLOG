...
public class CateServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int categoryNo = NumberUtils.toInt(request.getParameter("no"), 1);
		String categoryName = listDao.getCateNameByCateNo(categoryNo);
		...
		List<Product> products = listDao.getProductsByCateUpToDate(param);
				
		////
		int totalRows = listDao.getProductsByCateTotalRows(categoryNo);
		// 전체페이지
		...
		//event.jsp로 내부이동
		request.getRequestDispatcher("/WEB-INF/views/menus/category.jsp").forward(request, response);
	}
}
