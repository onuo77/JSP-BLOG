	...
	<div class="container-fluid h-100">
		...
		<%@include file="/WEB-INF/common/adminAside.jsp" %>
			<div class="col-10 h-100 side-content">
				...
				<h3 class="pt-4 pl-4">레시피 수정</h3>
				
				<div class="row justify-content-center mt-5 mb-5">
					<div class="col-10">
						<!--폼시작  -->
						<form action="/sb/recipemodify?recipeNo=${recipeInfo.no }" method="post" enctype="multipart/form-data">
							<div class="form-group">
							    <label>레시피 썸네일<span style="font-size:12px;">(사이즈 500*500px)</span></label>
							  	<div class="input-group mb-3">
									<input type="file" class="form-control" id="recipe-thum" name="newRecipeThum" accept="image/jpeg, image/png, image/gif" style="padding:3px;" required>
								</div>
							</div>
							<div class="form-group">
							    <label>레시피 제목</label>
							    <input type="text" class="form-control" value="${recipeInfo.title }" id="recipe-title" name="title" required/>
							</div>
							<div class="form-group">
							    <label>레시피 재료</label>
							    <textarea class="form-control" rows="2" id="recipe-ingredient" name="ingredient" required>${recipeInfo.ingredient }</textarea>
							</div>
							<div class="form-group">
							    <label>레시피 내용</label>
							    <textarea class="form-control" rows="10" placeholder="Enter Content" id="recipe-content" name="content" required>${recipeInfo.content }</textarea>
							</div>
							<div class="form-group">
								...
								<div class="border p-3 bg-light w-100 mt-2 rounded chkbox" id="category-1" style="display: none;">
									...
									<c:forEach var="product" items="${products1 }">
					                     <div class="form-check form-check-inline" style="width:15%;">
					                          <input class="form-check-input" type="checkbox" name="check-product" value="${product.productNo }" id="product-${product.productNo }" ${product.checked eq 'Y' ? 'checked' : '' }>
